...
<Html lang="en">
	<Preview
		preview="You were contacted by {first_name} {last_name} ({email}) using the More Info form on {import.meta
			.env.VITE_DOMAIN}"
	/>

	<Text
		>You were contacted by {first_name}
		{last_name} ({email}) using the More Info form on {import.meta.env.VITE_DOMAIN}.</Text
	>

	<Container>
		<Text>Mls: {mls}</Text>
		<Text>Show: {show.join(', ')}</Text>
		<Text>First name: {first_name}</Text>
		<Text>Last name: {last_name}</Text>
		<Text>Email: {email}</Text>
		<Text>Phone: {phone}</Text>
		<Text>Send me: {send_me.join(', ')}</Text>
